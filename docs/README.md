...
- **Wiring it into CI?** → [Provider setup](providers/index.md) has GitHub Actions, Gitea Actions, and Forgejo Actions examples per backend.
...
- **PR references are optional enrichment**, resolved from commit-message text where the merge strategy embeds it, with a live API fallback only on GitHub, where it's reliable. See [ADR-0001](adr/0001-pr-linkage-strategy.md).
...
| Git backends | GitHub, Gitea, Forgejo behind one interface | GitHub only | GitHub only | GitHub, GitLab, Bitbucket via official plugins; others via community plugins |
