...
`$` is Go's `text/template` root-data reference — inside the `{{range .Packages}}` loop, `.` is the current `PackagePlan`, so the single-mode branch (`{{if .Name}}` false, the implicit unnamed package) reaches back to the top-level `Plan.CompareURL` via `$.CompareURL` instead.

A separate `footer` attribution line is appended after this template runs (or not — see [`footer`](#footer) below); it isn't part of the template itself.
...
| `{{.PreviousTag}}` / `{{.SuggestedTag}}` | string | `PreviousVersion`/`SuggestedVersion` with `tag-format` applied — the actual git tag name, not bare SemVer. Empty wherever the corresponding `Version` field is. |
| `{{.CompareURL}}` | string | Link to a `PreviousTag..SuggestedTag` diff on the backend's web UI (single mode). Empty when either tag is empty, or no `Backend` was configured — guard with `{{if .CompareURL}}`. |
...
| `{{.Packages}}[].PreviousTag` / `{{.Packages}}[].SuggestedTag` | string | Same as the top-level `PreviousTag`/`SuggestedTag`, but for this Package. |
| `{{.Packages}}[].CompareURL` | string | Same as the top-level `CompareURL`, but for this Package. |
...
Whether draftsman appends its own attribution line — `*Release notes generated by [draftsman](https://brpaz.github.io/draftsman/).*` — after every rendered changelog body (the top-level body and, in `multi` mode, each Package's own isolated body). Set `footer: false` to omit it. This is independent of `template`: it's appended after the template runs, not part of it, so it applies (or doesn't) the same way regardless of whether `template` is customized.
