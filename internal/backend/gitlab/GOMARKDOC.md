...
GitRemoteURL implements backend.Backend. "oauth2" is GitLab's documented placeholder username for token\-based HTTPS auth.
...
### func \(\*Client\) [UpsertReleasePR](<https://github.com/brpaz/draftsman/blob/main/internal/backend/gitlab/gitlab.go#L161>)

```go
func (c *Client) UpsertReleasePR(context.Context, backend.UpsertReleasePRRequest) (backend.ReleasePR, error)
```

UpsertReleasePR implements backend.Backend. Not yet implemented — release\-strategy: pr's ticket breakdown \(.scratch/pr\-release\-strategy/\) scopes real adapter work to GitHub, Gitea and Forgejo; GitLab uses merge requests rather than pull requests and isn't covered by any ticket yet.
