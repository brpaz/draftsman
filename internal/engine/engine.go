...
// configured template. PreviousVersion/SuggestedVersion (and their Tag/
// CompareURL counterparts) are the repo-wide version (single mode only —
// see PackagePlan for multi mode).
type Plan struct {
...
func Compute(ctx context.Context, repoPath string, cfg *config.Config, be backend.Backend) (*Plan, error) {
	...
	rendered, err := RenderPlan(cfg.Template, plan, cfg.FooterEnabled())
	if err != nil {
		return nil, err
	}
...
// footerText is appended to every rendered changelog body when a Config's
// Footer is true (the default) — draftsman's own attribution, disabled
// with "footer: false" in .draftsman.yml.
...
// changelog body text, then appends footerText when footer is true. Compute
// uses this for the full repo-wide/multi Plan; multi-mode draft/publish
// commands reuse it to render an isolated body for a single Package's own
// draft release, by wrapping that one PackagePlan in a synthetic *Plan —
// footer is threaded through separately each time so every rendered body
// gets (or skips) it consistently, not just the top-level one.
...
func computeSingle(ctx context.Context, repoPath string, cfg *config.Config, be backend.Backend, format *version.Format, sectionOrder []string) (*Plan, error) {
	...
	if hasPrevious {
		...
		plan.PreviousTag = format.Render(plan.PreviousVersion, "")
	}
	switch {
	...
	if plan.SuggestedVersion != "" {
		plan.SuggestedTag = format.Render(plan.SuggestedVersion, "")
	}
	if be != nil && plan.PreviousTag != "" && plan.SuggestedTag != "" {
		plan.CompareURL = be.CompareURL(plan.PreviousTag, plan.SuggestedTag)
	}
...
func computeMulti(ctx context.Context, repoPath string, cfg *config.Config, be backend.Backend, format *version.Format, sectionOrder []string) (*Plan, error) {
	...
	for _, pkg := range cfg.Packages {
		...
		if hasPrevious {
			...
			pp.PreviousTag = format.Render(pp.PreviousVersion, pkg.Name)
		}
		switch {
		case bump == version.BumpNone:
		case hasPrevious:
		...
		if pp.SuggestedVersion != "" {
			pp.SuggestedTag = format.Render(pp.SuggestedVersion, pkg.Name)
		}
		if be != nil && pp.PreviousTag != "" && pp.SuggestedTag != "" {
			pp.CompareURL = be.CompareURL(pp.PreviousTag, pp.SuggestedTag)
		}
