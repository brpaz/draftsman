...
func TestCompute_FooterAppendedByDefault(t *testing.T) {
	repo := initRepo(t, "feat: add login page")

	plan, err := engine.Compute(context.Background(), repo, config.Default(), nil)
	require.NoError(t, err)

	require.Contains(t, plan.Rendered, "Release notes generated by [draftsman]")
}

func TestCompute_FooterFalseOmitsIt(t *testing.T) {
	repo := initRepo(t, "feat: add login page")
	...
	require.NotContains(t, plan.Rendered, "Release notes generated by [draftsman]")
}
